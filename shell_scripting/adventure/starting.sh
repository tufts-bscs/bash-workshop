...
read _______ # (Hint: Store the player's name here)
echo "Hello, $_______! Let's begin your adventure."
...
while ! [[ "$choice" =~ ^[1-3]$ ]]; do
    ...
    read _______ # (Hint: Store the player's choice here)
done
...
if [ "$choice" -eq 1 ]; then
    ...
    read minutes
    while [ "$minutes" _________ ]; do # (Hint: What condition should be checked here?)
        echo "Searching... Time left: $minutes minutes"
        _________ # (Hint: How do you decrease the value of 'minutes'?)
        sleep 1
    done
...
if [ "$play_again" = "yes" ]; then
    ...
    echo "Thanks for playing, $_______! Goodbye!" # (Hint: Use the player's name here)
    exit 0
fi
